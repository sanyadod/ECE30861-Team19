...
def test_calculate_net_score(temp_config):
    ...
    metric_results = {
        ...
        'size_score': SizeScore(raspberry_pi=0.5, jetson_nano=0.7, desktop_pc=0.9, aws_server=1.0),
...
async def test_score_model_integration(temp_config, model_context):
    ...
    for metric in scorer.metrics:
        if metric.name == 'size_score':
            scorer._compute_size_metric_with_latency = AsyncMock(return_value=(
                SizeScore(raspberry_pi=0.5, jetson_nano=0.7, desktop_pc=0.9, aws_server=1.0),
                300
            ))
        else:
