...
async def test_hf_api_file_list_success():
    ...
    with patch.object(api.api, 'model_info') as mock_model_info, \
         patch('src.hf_api.list_repo_files') as mock_list_files:
        ...
        mock_list_files.return_value = ["config.json", "pytorch_model.bin", "tokenizer.json"]
...
@pytest.mark.parametrize("readme,expected_license", [
