...
from src.models import ModelContext, ParsedURL, URLCategory, MetricResult, SizeScore
...
async def test_full_scoring_pipeline():
    ...
    # Mock HF API calls
    scorer.hf_api.get_model_info = AsyncMock(return_value={'id': 'test/model', 'downloads': 100})
    scorer.hf_api.get_readme_content = AsyncMock(return_value="# Test Model\n## Usage\nExample usage")
    ...
    for metric in scorer.metrics:
        if metric.name == 'size_score':
            metric._calculate_size_scores = AsyncMock(return_value=SizeScore(
                raspberry_pi=0.5, jetson_nano=0.7, desktop_pc=0.9, aws_server=1.0
            ))
        else:
            metric.compute = AsyncMock(return_value=MetricResult(score=0.7, latency=100))
...
def test_process_urls_integration(mock_logging, mock_contexts, mock_asyncio, sample_url_file):
...
def test_error_handling_graceful():
    ...
    context = ModelContext(model_url=ParsedURL(
    ...
    async def test_error_resilience():
        ...
        scorer.hf_api.get_readme_content = AsyncMock(side_effect=Exception("README Error"))
        scorer.hf_api.get_model_config = AsyncMock(side_effect=Exception("Config Error"))
