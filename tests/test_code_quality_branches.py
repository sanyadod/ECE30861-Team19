...
def test_analyze_code_repository_scoring_mix():
    ...
    file_score = metric._calculate_file_quality_score(analysis['file_analysis'], {'min_test_coverage': 0.5})
    commit_score = metric._calculate_commit_quality_score(analysis['commit_analysis'])

    # Combine as in _analyze_code_repository
    combined = min(1.0, 0.25 * analysis['structure_analysis']['structure_score']
...
def test_calculate_commit_quality_score_branches():
    ...
    # High activity path
    s = metric._calculate_commit_quality_score({'total_commits': 25, 'recent_commits': 6, 'avg_commit_frequency': 0.15})
    # 0.3 + 0.3 + 0.4 = 1.0
    assert abs(s - 1.0) < 1e-6

    # Lower activity path
    s = metric._calculate_commit_quality_score({'total_commits': 6, 'recent_commits': 1, 'avg_commit_frequency': 0.05})
    # 0.2 + 0.2 + 0.2 = 0.6
    assert abs(s - 0.6) < 1e-6
