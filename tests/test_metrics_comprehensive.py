...
async def test_bus_factor_no_data(config):
    ...
    context = ModelContext(
        model_url=ParsedURL(url="https://huggingface.co/test/model", category=URLCategory.MODEL, name="test/model", platform="huggingface")
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("readme_content,expected_range", [
    ("No performance data", (0.0, 0.2)),
    ("GLUE benchmark: 82.3% accuracy", (0.3, 0.6)),
    ("MMLU score: 0.85\nSee paper: https://arxiv.org/abs/2301.00001", (0.6, 1.0)),
    ("Performance on GLUE: 85.2%\nMMLU: 0.87\nCitation: [Paper](https://link.com)", (0.7, 1.0))
])
...
async def test_size_score_name_patterns(model_name, expected_size, config):
    ...
    context = ModelContext(
        model_url=ParsedURL(url=f"https://huggingface.co/test/{model_name}", category=URLCategory.MODEL, name=f"test/{model_name}", platform="huggingface"),
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("has_datasets,has_code,expected_min", [
...
async def test_dataset_code_combinations(has_datasets, has_code, expected_min, config):
    ...
    if has_datasets:
        context.datasets = [ParsedURL(url="https://huggingface.co/datasets/test/data", category=URLCategory.DATASET, name="test/data", platform="huggingface")]
    if has_code:
        context.code_repos = [ParsedURL(url="https://github.com/test/code", category=URLCategory.CODE, name="test/code", platform="github")]
...
async def test_dataset_quality_with_mock_hf_dataset(config):
    ...
    with patch('src.metrics.dataset_quality.HuggingFaceAPI') as MockAPI:
        ...
        mock_api_instance.get_readme_content = AsyncMock(return_value="""
        # Dataset Description
        ...
        """)
        mock_api_instance.get_dataset_info = AsyncMock(return_value={'tags': ['license:apache-2.0']})
...
async def test_code_quality_with_mock_analysis(config):
    ...
    with patch('src.metrics.code_quality.GitInspector') as MockInspector:
        ...
        mock_inspector.analyze_repository.return_value = {
            ...
            'file_analysis': {'python_files': 20, 'test_files': 10, 'total_lines_of_code': 5000},
            'commit_analysis': {'total_commits': 50, 'recent_commits': 10, 'avg_commit_frequency': 0.5}
        }
