        ...
        for match in matches:
            try:
                ...
                if unit == "B":  # Billion parameters
                    return size_value * 2.0  # ~2GB per billion parameters (rough estimate)
...
def parse_license_from_readme(readme_content: str) -> Optional[str]:
    ...
    for pattern in license_patterns:
        ...
        if match:
            ...
            license_text = re.sub(r'\[([^\]]+)\]\([^\)]+\)', r'\1', license_text)  # Remove markdown links
            return license_text[:200]  # Limit length
    
    return None


def check_readme_sections(readme_content: str, required_sections: List[str]) -> Dict[str, bool]:
    """
...
def extract_performance_claims(readme_content: str, benchmark_keywords: List[str]) -> Dict[str, Any]:
    """
    ...
    """
    if not readme_content:
        return {'benchmarks_mentioned': [], 'numeric_results': False, 'citations': False}
    ...
    has_citations = any(re.search(pattern, readme_lower) for pattern in citation_patterns)
