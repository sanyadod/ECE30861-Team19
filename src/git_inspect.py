...
class GitInspector:
    ...
    def clone_repo(self, repo_url: ParsedURL) -> Optional[str]:
        ...
        repo_name = f"{repo_url.owner}_{repo_url.repo}" if repo_url.owner and repo_url.repo else "unknown"
        ...
        try:
            ...
            porcelain.clone(repo_url.url, clone_path, depth=50)  # Shallow clone for efficiency
            return clone_path
    ...
    def _analyze_commits(self, repo: Repo) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'time_span_days': (max(commit_dates) - min(commit_dates)) / (24 * 60 * 60) if len(commit_dates) > 1 else 0
            }
    ...
    def _analyze_contributors(self, repo: Repo) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'unique_authors': 0, 'unique_committers': 0, 'bus_factor_score': 0.0}
    ...
    def _analyze_files(self, repo_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            test_files = [f for f in python_files if 'test' in f.name.lower() or f.parent.name.lower() in ['test', 'tests']]
            ...
            return {
                ...
                'test_coverage_estimate': len(test_files) / max(1, len(python_files)) if python_files else 0
            }
            
        except Exception as e:
            logger.warning(f"Error analyzing files: {e}")
            return {'total_files': 0, 'python_files': 0, 'test_files': 0, 'total_lines_of_code': 0}
    ...
    def _analyze_structure(self, repo_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            has_license = any(repo_root.glob("LICENSE*")) or any(repo_root.glob("LICENCE*"))
            has_requirements = (repo_root / "requirements.txt").exists() or (repo_root / "pyproject.toml").exists()
            has_setup = (repo_root / "setup.py").exists() or (repo_root / "setup.cfg").exists()
            has_ci = (repo_root / ".github").exists() or (repo_root / ".gitlab-ci.yml").exists()
            
            structure_score = sum([has_readme, has_license, has_requirements, has_setup, has_ci]) / 5.0
    ...
    def _analyze_documentation(self, repo_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_score = sum([bool(readme_content), has_usage, has_installation, has_examples]) / 4.0
    ...
    def _empty_analysis(self) -> Dict[str, Any]:
        ...
        return {
            'commit_analysis': {'total_commits': 0, 'recent_commits': 0, 'avg_commit_frequency': 0},
            ...
            'documentation_analysis': {'documentation_score': 0.0, 'readme_content': ''}
        }
