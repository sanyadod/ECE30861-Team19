...
class HuggingFaceAPI:
    ...
    async def get_model_info(self, model_url: ParsedURL) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            repo_id = f"{model_url.owner}/{model_url.repo}" if model_url.owner else model_url.repo
    ...
    async def get_dataset_info(self, dataset_url: ParsedURL) -> Optional[Dict[str, Any]]:
        """Get dataset information from HF Hub API."""
        if dataset_url.platform != "huggingface" or not dataset_url.repo:
            return None
        
        try:
            repo_id = f"{dataset_url.owner}/{dataset_url.repo}" if dataset_url.owner else dataset_url.repo
    ...
    async def download_file(self, repo_id: str, filename: str, is_dataset: bool = False) -> Optional[str]:
    ...
    async def get_readme_content(self, parsed_url: ParsedURL) -> Optional[str]:
        ...
        repo_id = f"{parsed_url.owner}/{parsed_url.repo}" if parsed_url.owner else parsed_url.repo
