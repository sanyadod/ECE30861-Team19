...
from .models import ParsedURL, URLCategory, ModelContext
...
def _parse_huggingface_url(url: str, parsed) -> ParsedURL:
    ...
    if len(path_parts) >= 2 and path_parts[0] == "datasets":
        ...
        name = f"{owner}/{repo}" if owner and repo else url.split("/")[-1]
    else:
...
def _find_relevant_resources(model_url: ParsedURL, resources: List[ParsedURL]) -> List[ParsedURL]:
    """
    ...
    for resource in resources:
        ...
        if (model_name_parts & resource_name_parts or 
            model_url.owner == resource.owner):
